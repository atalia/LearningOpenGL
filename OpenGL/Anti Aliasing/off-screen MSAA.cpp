...
int main()
{
	...
	//相对于非MSAA的渲染，这里需要将多采样的color buffer resolve
	...
	if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
		std::cout << "ERROR::FRAMEBUFFER:: screen frame buffer is not complete!" << std::endl;
	glBindFramebuffer(GL_FRAMEBUFFER, 0);


	//std::cout << sizeof(GLfloat) << ", " << sizeof(GL_FLOAT) << std::endl;
